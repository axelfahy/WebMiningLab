...
public class WebPageIndexerQueue extends Thread {
    ...
    public boolean queueVisitedPage(VisitedPage page) {
        ...
        if (!page.isComplete())
            throw new RuntimeException("The visited page is not complete " + page.getMetadata().getUrl());
    ...
    public void run() {
        while (!allDone || queueToIndex.size() > 0) {
            ...
            if (Labo1.DEBUG)
                System.err.println("INDEXER: remaining: " + queueToIndex.size() + " [" + (allDone ? "waiting for finish" : "waiting for data") + "]");
